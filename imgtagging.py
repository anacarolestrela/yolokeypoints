...
def process_image(image_path, scale_percent=50):
    ...
    results = detector(resized_image)
    keypoints = detector.get_xy_keypoint(results)
    
    if keypoints:
        for name, (x, y) in keypoints.items():
            if x is not None and y is not None:
                x_int = int(x * resized_image.shape[1])
                y_int = int(y * resized_image.shape[0])
                cv2.circle(resized_image, (x_int, y_int), 5, (0, 255, 0), -1)
                cv2.putText(resized_image, name, (x_int, y_int - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    
    cv2.imshow('YOLOv8 Pose Detection', resized_image)
    cv2.waitKey(0)  # Aguarda o usuário pressionar qualquer tecla
    cv2.destroyAllWindows()

if __name__ == "__main__":
    process_image('Dataset_piloto/takeoff/IMG_20240529_161622.jpg', scale_percent=30)  
